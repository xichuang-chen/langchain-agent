...
用户：我是小逸，我喜欢拿铁
Agent: 原来你是小逸呀😊，喜欢热的拿铁确实很温暖又香醇。拿铁的奶香和咖啡的浓香融合在一起，非常适合慢慢品味。你一般喜欢在什么时间喝热拿铁呢？是早晨提神，还是下午小憩的时候？

stop the agent.
npm run start again.

用户: 你还记得我是谁，喜欢什么吗
Agent: 我现在并不能记住你之前跟我说过的话，也不了解你过去的喜好或身份信息。  \n不过你可以告诉我你是谁、你喜欢什么，我可以根据你提供的信息来和你交流，并在这次对话中记住它。  \n\n你愿意先跟我分享一下你的兴趣爱好吗？
...
用户: 你还记得我是谁，喜欢什么吗
Agent: 当然记得呀😊  \n你是 **小逸**，喜欢 **热咖啡**，尤其是 **拿铁**。  \n拿铁的奶香和咖啡的浓香融合在一起，非常适合慢慢品味。  \n\n你想让我帮你推荐几款适合你口味的拿铁特别配方吗？
用户: 你还记得我喜欢什么动物吗
Agent: 记得呀🐶  \n你喜欢 **小狗**，觉得它们的陪伴温暖又治愈。  \n\n你是喜欢活泼好动的品种，比如柯基、拉布拉多，还是乖巧黏人的，比如博美、马尔济斯呢？
```

# 遇到的问题

## 1. 当我问的问题触发了Azure/LiteLLM 的 Content Policy时，会报错


<details>

<summary>展开查看错误信息摘要</summary> 
...
### 原因分析：  
意思是：
发送的 prompt 或系统消息触发了 Azure OpenAI / LiteLLM 的内容安全策略

请求被拒绝，返回 400 BadRequest
...
- 没有可用的内容策略 fallback 模型
- Fallbacks=[...] LiteLLM 尝试过一些备选模型，但都没能自动切换


**解决办法：**  
我也不清楚为什么 "我喜欢咖啡" 会触发omina gpt-5-chat Azure OpenAI's content management policy  

所以，我们需要一个fallback 的模型 gpt-5-mini-2025-08-07  

最终的解决方案为，在main.ts 中，当发生错误时，fallback 
...
问题描述:  
〉 无法使用本地文件存储的方式 嵌入式（本地持久化）方案

在 Node/TS 里 chromadb 包本质是 HTTP client，@langchain/community 的 Chroma 也会去连服务端；真正“embedded + 本地持久化”更偏 Python 生态
...
错误日志  
```
Agent 执行出错: BadRequestError: 400 litellm.ContextWindowExceededError: litellm.BadRequestError: AzureException ContextWindowExceededError - This model's maximum context length is 8192 tokens, however you requested 15615 tokens (15615 in your prompt; 0 for the completion). Please reduce your prompt; or completion length.. Received Model Group=text-embedding-3-small-1
Available Model Group Fallbacks=None
Error doing the fallback: litellm.ContextWindowExceededError: litellm.BadRequestError: AzureException ContextWindowExceededError - This model's maximum context length is 8192 tokens, however you requested 15615 tokens (15615 in your prompt; 0 for the completion). Please reduce your prompt; or completion length. LiteLLM Retried: 3 times
```  


从你贴的终端日志看，报错点在向向量库写入长期记忆时做 embedding，把一大段（15k+ tokens）的文本丢给 text-embedding-3-small 导致超过 8192 上限

解决方案:

设置 max chars 当 embedding 到向量数据库时
